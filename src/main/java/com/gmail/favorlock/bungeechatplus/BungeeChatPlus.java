...
public class BungeeChatPlus extends Plugin {
	...
	private void loadRules() {
		File directory = new File("plugins" + File.separator + this.getDescription().getName());
    ...
    private void compilePattern(String re) {
    	...
    	if (patterns.get(re) == null) {
    		try {
    			...
    			this.getProxyServer().getLogger().fine("Successfully compiled regex: " + re);
    		}
    		catch (PatternSyntaxException e) {
    			this.getProxyServer().getLogger().warning("Failed to compile regex: " + re);
    			this.getProxyServer().getLogger().warning(e.getMessage());
    		}
    		catch (Exception e) {
    			this.getProxyServer().getLogger().severe("Unexpected error while compiling expression '" + re + "'");
    			e.printStackTrace();
    		}
    	}
    }
    ...
    public void filterChat(ChatEvent event) {
        ...
        if (!(player.hasPermission("BungeeChatPlus.bypass"))) {
	    	...
	    	String warnmsg = getConfig().Settings_warnmsg;
	    	String kickmsg = getConfig().Settings_kickmsg;
	    	...
	    	for (String line : this.rules) {
	    		...
	    		if (matched) {  
		    		...
		    		if (valid == false) {
		    			this.getProxyServer().getLogger().warning("Ignored syntax error in rules.txt: " + line);    			
		    		}	    		
	    		}
	    	}	
    ...
    public void logToFile(String message) {   
    	...
    	this.getProxyServer().getLogger().info(message);  	
    	Boolean logEnabled = getConfig().Settings_EnableLog;
    	if (logEnabled) {	
	    	try {
	    		File directory = new File("plugins" + File.separator + this.getDescription().getName());
